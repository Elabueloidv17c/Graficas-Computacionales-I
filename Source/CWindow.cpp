...
#ifdef OPEN_GL
void CWindow::Initialize(Rect dimensions, unsigned int displayMode, const char* name, Color color, std::vector <const char*> scene)
{
	//Window
...
void CWindow::Render(CShaderProgram& shaderProgram)
{	
	for (int i = 0; i < m_scene.m_models.size(); i++)
	{
		...
		{
			m_scene.Render(shaderProgram, *m_camera.GetActiveCamera());
		}
	}
}
