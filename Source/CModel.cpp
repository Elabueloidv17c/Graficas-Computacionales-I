...
#ifdef OPEN_GL
bool CModel::Initialize(const char* modelName)
{
	m_modelPath = modelName;

	//Load mesh from file
	const aiScene* model = aiImportFile(modelName, aiProcessPreset_TargetRealtime_MaxQuality);
	...
	for (int i = 0; i < m_meshes.size(); i++)
	{
		...
		if (model->HasMaterials())
		{
			...
			for (unsigned int i = strlen(modelName) - 1; i >= 0; i--)
			{
				if (modelName[i] == '/')
				{
					for (unsigned int j = 0; j <= i; j++)
					{
						textureName.append(&modelName[j], 1);
					}
...
void CModel::Render(CShaderProgram& shaderProgram, CCamera& camera)
{
	...
	for (int i = 0; i < m_meshes.size(); i++)
	{
		for (int j = 0; j < m_meshes[i].m_transforms.size(); j++)
		{
			if (m_modelPath == "../Anubis/Anubis.fbx")
			{
				...
				glm::radians(-camera.m_yaw + 90.0f), VECTOR3(0.0f, 1.0f, 0.0f)), glm::radians(-camera.m_pitch), VECTOR3(1.0f, 0.0f, 0.0f));
			}
...
#ifdef DIRECT_X
bool CModel::Initialize(const char* modelName, CDevice& device)
{
	m_modelPath = modelName;
		...
		if (filePath.length > 1)
		{
			for (unsigned int i = strlen(modelName) - 1; i >= 0; i--)
			{
				if (modelName[i] == '/')
				{
					for (unsigned int j = 0; j <= i; j++)
					{
						texturePath.append(&modelName[j], 1);
					}
	...
	for (int i = 0; i < m_meshes.size(); i++)
	{
		...
		for (int j = 0; j < m_meshes[i].m_transforms.size(); j++)
		{
			if (m_modelPath == "../Anubis/Anubis.fbx")
			{
				...
				else if (otherCamera.m_viewMatrix != camera.m_viewMatrix)
				{
